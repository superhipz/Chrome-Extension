...
const performanceAssetFilterList = [file => !/\.map$/.test(file), file => !file.endsWith('.zip'), file => !/^icons\//.test(file)]
...
module.exports = (api, options) => {
    ...
    api.chainWebpack(webpackConfig => {
        ...
        webpackConfig.output.filename(
            file => `js/[name]${isLegacyBundle ? `-legacy` : ``}${isProduction && options.filenameHashing && !userScripts.includes(file.chunk.name) ? '.[contenthash:8]' : ''}.js`
